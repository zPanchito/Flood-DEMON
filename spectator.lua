...
ACT_FOLLOW_SPECTATOR = allocate_mario_action(ACT_GROUP_AIRBORNE | ACT_FLAG_INVULNERABLE)
...
function set_mario_spectator(m)
    if eFloodVariables.spectatorMode == SPECTATOR_MODE_NORMAL then
        if m.action ~= ACT_SPECTATOR then
            ...
            set_mario_action(m, ACT_SPECTATOR, 0)
        end
    elseif eFloodVariables.spectatorMode == SPECTATOR_MODE_FOLLOW then
        set_mario_action(m, ACT_FOLLOW_SPECTATOR, 0)
    end
end
...
local function act_follow_spectator(m)
    mario_drop_held_object(m)
    m.squishTimer = 0
    ...
    if not is_game_paused() then
        if m.controller.buttonPressed & R_JPAD ~= 0 or targetPlayer == 0 then
            local originalIndex = targetPlayer
            targetPlayer = targetPlayer + 1

            while not gNetworkPlayers[targetPlayer].connected
            or gPlayerSyncTable[targetPlayer].finished do
                targetPlayer = targetPlayer + 1

                if targetPlayer >= MAX_PLAYERS then
                    targetPlayer = originalIndex
                    break
                end
            end
        end
    ...
    local tM = gMarioStates[targetPlayer]

    -- set mario's position to the target player's position
    vec3f_copy(m.pos, tM.pos)
    -- reset camera overrides
    camera_reset_overrides()
    -- reset camera near, far, and fov
    set_override_near(0)
    set_override_far(0)
    set_override_fov(0)
    -- unfreeze camera
    camera_unfreeze()
    -- force freecam, as it's an oribiting cam
...
local function on_set_mario_action(m)
    ...
    if m.action == ACT_SPECTATOR then
        ...
        sPlayerFirstPerson.yaw = m.faceAngle.y + 0x8000
    elseif m.action == ACT_FOLLOW_SPECTATOR then
        sPlayerFirstPerson.freecam = camera_config_is_free_cam_enabled()
    else
        camera_unfreeze()
        set_override_near(0)
...
hook_mario_action(ACT_FOLLOW_SPECTATOR, act_follow_spectator)