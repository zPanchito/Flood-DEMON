-- code created by Gaming32, modified by zPancho!
...
--[[
		elseif m.action == ACT_GROUP_MASK then
            message = "%s I was AFK"
		elseif m.action == ACT_GROUP_AIRBORNE then
            message = "%s I was AFK"
--]]
        ...
        elseif m.action == ACT_DEATH_ON_BACK or m.action == ACT_DEATH_ON_STOMACH then
            if hurtMario ~= nil then
                ...
                hurtMario = nil
                message = "%s was eliminated by " .. ememyName .. "."
            else
                message = "%s he slipped form a very high place"
            end
        elseif m.floor.type == SURFACE_DEATH_PLANE and m.pos.y < m.floorHeight + 2048 then
            hasDied = true
            message = "%s fell off the map in " .. name_of_level(gLevels[gGlobalSyncTable.level].level, gLevels[gGlobalSyncTable.level].area, gLevels[gGlobalSyncTable.level].name, gLevels[gGlobalSyncTable.level]) .. "."
        end
